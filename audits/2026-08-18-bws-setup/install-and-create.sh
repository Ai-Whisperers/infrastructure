...
# Re-extract if not present (check both bitwarden_sdk AND bitwarden_py)
if [[ ! -d "$LIB/bitwarden_sdk" ]] || [[ ! -f "$LIB/bitwarden_sdk/__init__.py" ]] \
   || [[ ! -d "$LIB/bitwarden_py" ]] || [[ ! -f "$LIB/bitwarden_py/bitwarden_py.abi3.so" ]]; then
    ...
    # Clean target dir then copy all needed packages
    rm -rf "$LIB"/*
